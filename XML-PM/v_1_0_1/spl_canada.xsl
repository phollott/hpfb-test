...
<!--		[pmh to do]			<xsl:choose>
						<xsl:when test="string-length($product-description) - string-length(translate($product-description, '-', '')) &lt; 2">
							<xsl:value-of select="replace($product-description, ' - ', '')"/>
						</xsl:when>
						<xsl:otherwise> -->
